...
You can download the raw source code for these lecture notes [here](compmus2026-w12.qmd).
...
### Wednesday · 18 March · Lecture
  ...
  - Lecture: Generative AI for music (10 minutes)
  - Exam info: (5 minutes)
  - Wrap-up: (10 minutes)
...
The *NRC*, one of the Dutch national newspapers, maintains a [Spotify playlist](https://open.spotify.com/playlist/5tuYtDM305tknEs94i8F5P) that their music editors feel represents the best new music of the moment.
Perhaps clustering can help us organise and describe what kinds of musical selections make it onto the list.
You can download an [Exportify](https://exportify.net) CSV of this list [here](../data/nrc.csv).
...
nrc_juice <-
  ...
  prep(nrc |> mutate(`Track Name` = str_trunc(`Track Name`, 36))) |>
  juice() |>
...
In order to demonstrate some of the principles of classification, we will try to identify some of the features that Spotify uses to designate playlists as 'workout' playlists. For a full analysis, we would need to delve deeper, but let's start with a comparison of three playlists: Indie Pop, Indie Party, and Indie Running For speed, this example will work with only the first 20 songs from each playlist, but you should feel free to use more if your computer can handle it. You can download the Exportify CSVs here:
  - [Indie Pop](../dat/indie-pop.csv)
  - [Indie Party](../dat/indie-party.csv)
  - [Indie Running](../dat/indie-running.csv)
...
```{r}
pop <- read_csv("../dat/indie-pop.csv", col_types = "ccccDnnlcTccnnnnnnnnnnnn")
party <- read_csv("../dat/indie-party.csv", col_types = "ccccDnnlcTccnnnnnnnnnnnn")
workout <- read_csv("../dat/indie-running.csv", col_types = "ccccDnnlcTccnnnnnnnnnnnn")
indie <-
  bind_rows(
